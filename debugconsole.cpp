...
bool FuncDef::Call(char* func, CVarConsole* console, char args[ARG_BUFFER][STR_BUFFER], int args_n) {
		...
		if(!strcmp(func, "print_s")) {
			for(int x=1; x<args_n; x++) {
				char buffer[256];
				sprintf(buffer, "%s ", args[x]);
				debug->statusPrint(buffer);
			}
			return true;
		}
		...
		if(!strcmp(func, "draw")) {
			...
			if(!CMap::instance()->DoesExist(var)) {
				char buffer[256];
		...
		if(!strcmp(func, "init")) {
			CInt players_n("players_n");
			if(*players_n < 2) {
				console->print("$players_n larger than 2 is required to play the game");
				return true;
			}
...
DebugConsole::DebugConsole(Tmpl8::Game* game_n, int max_lines_num_n, float pos_x_n, float pos_y_n, float size_x_n) : SceneNode(SceneManager::instance()->getRoot()) {
	game = game_n;
//game->addNode(this);
...
void DebugConsole::drawSelf(Tmpl8::Surface* screen) {
	...
	int counter = 0;
	screen->Box(pos_x, pos_y, pos_x+size_x, pos_y+20.f*max_lines_num, 0xffffffff);
...
void DebugConsole::drawVector(const vec2& v) {
...
void DebugConsole::statusPrint(char* msg) {
	game->getStatus()->addMsg(msg);
}

StatusConsole::StatusConsole(Tmpl8::Game* game_n, int max_lines_num_n, float pos_x_n, float pos_y_n, float size_x_n) : SceneNode(SceneManager::instance()->getRoot()) {
	game = game_n;
	max_lines_num = max_lines_num_n;
	pos_x = pos_x_n;
	pos_y = pos_y_n;
...
void StatusConsole::set(int max_lines_num_n, float pos_x_n, float pos_y_n, float size_x_n) {
	max_lines_num = max_lines_num_n;
	pos_x = pos_x_n;
	pos_y = pos_y_n;
	size_x = size_x_n;
...
void StatusConsole::addMsg(char* text) {
	history.push_back(text);
	if(history.size() > max_lines_num)
		history_pos++;
}

void StatusConsole::update(float frameDelta) {

}

void StatusConsole::drawSelf(Tmpl8::Surface* screen) {
	/*Tmpl8::Surface* bg = new Tmpl8::Surface(size_x, 16.f*max_lines_num);
	bg->Clear(0x001947);
	bg->CopyTo(screen, pos_x, pos_y);
	delete bg;*/

	int counter = 0;
	//screen->Box(pos_x, pos_y, pos_x+size_x, pos_y+16.f*max_lines_num, 0x222222);
	for(std::list<char*>::iterator x = history.begin(); x != history.end(); x++) {
		if(counter >= history_pos) {
			Tmpl8::Surface* tmp = new Tmpl8::Surface(512, 20);
			tmp->InitCharset();
