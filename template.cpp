...
{  
	...
	if (MessageBox(NULL, "Run in fullscreen?", "Icebreaker v1.0", MB_YESNO) == IDYES) {
	...
	if (!init()) 
	{
		...
		surface = new Surface( SCRWIDTH, SCRHEIGHT, (Tmpl8::Pixel*)s->pixels, s->pitch );
		surface->InitCharset();
		vbo = false;
		SDL_WM_SetCaption( "Icebreaker v1.0 by Alexander Dzhoganov", NULL );
	}
	else SDL_WM_SetCaption( "Icebreaker v1.0 by Alexander Dzhoganov", NULL );
	int exitapp = 0;
	game = new Game();
	...
	while (!exitapp) 
	{
		...
		{
			swap();
			surface->SetBuffer( (Tmpl8::Pixel*)framedata );
		}
