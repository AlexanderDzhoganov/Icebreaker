#pragma warning(disable : 4172 4530 4018 4244)
...
bool CVarConsole::parseIdentifier(char* input) {
	...
	bool vcount = CMap::instance()->DoesExist(input);
	if(!vcount) {
		...
		CType type = CMap::instance()->GetType(input);
		if(type == CType_int) {
			CInt tmp(input);
			char buffer[STR_BUFFER];
			sprintf(buffer, "(int) %s = %i (ref: %i)\n", input, *tmp, CMap::instance()->GetRef(&tmp)-1);
			std::string out(buffer);
			out_buffer.push_back(out);
			...
			CVec2 tmp(input);
			char buffer[STR_BUFFER];
			sprintf(buffer, "(vec2) %s = [%f, %f] (ref: %i)\n", input, (*tmp)[0], (*tmp)[1], CMap::instance()->GetRef(&tmp)-1);
			std::string out(buffer);
			out_buffer.push_back(out);
		}
	}
...
bool CVarConsole::parseCommand(char* input) {
	...
	if(!strcmp(command, "list")) {
		std::list<std::string> names = CMap::instance()->GetVars();
		for(std::list<std::string>::iterator x = names.begin(); x != names.end(); x++) {
			char* name = (char*)(*x).c_str();
			CType type = CMap::instance()->GetType(name);
			if(type == CType_int) {
				CInt tmp(name);
				char buffer[STR_BUFFER];
				sprintf(buffer, "(int) %s = %i (ref: %i)\n", name, *tmp, CMap::instance()->GetRef(&tmp)-1);
				std::string out(buffer);
				out_buffer.push_back(out);
			} else if(type == CType_float) {
				CFloat tmp(name);
				...
				CVec2 tmp(name);
				char buffer[STR_BUFFER];
				sprintf(buffer, "(vec2) %s = [%f, %f] (ref: %i)\n", name, (*tmp)[0], (*tmp)[1], CMap::instance()->GetRef(&tmp)-1);
				std::string out(buffer);
				out_buffer.push_back(out);
			}
		}
	...
	if(!strcmp(command, "declare")) {
		...
		bool vcount = CMap::instance()->DoesExist(var);
		if(vcount) {
			char buffer[STR_BUFFER];
			...
			CType type = CMap::instance()->GetTypeFromStr(strtype);
			if(type == CType_int) {
...
/*	if(!strcmp(command, "call")) {
	...
	if(!strcmp(command, "add")) {
		...
		if(parsed_b) {
			var_b = parsed_b;
			if(!CMap::instance()->DoesExist(var_b)) {
				char buffer[STR_BUFFER];
			...
			CType type_b = CMap::instance()->GetType(var_b);
			if(type_b == CType_int) {
		...
		if(!CMap::instance()->DoesExist(var_a)) {
			char buffer[STR_BUFFER];
		...
		CType type = CMap::instance()->GetType(var_a);
		if(type == CType_int) {
			CInt tmp(var_a);
			*tmp += atoi(var_b);
			return true;
...
bool CVarConsole::parseAssignment(char* input) {
	...
	if(varname) {
		if(!CMap::instance()->DoesExist(varname)) {
			char buffer[STR_BUFFER];
			sprintf(buffer, "Identifier not found \"%s\"\n", varname);
			std::string out(buffer);
			out_buffer.push_back(out);
		...
		if(CMap::instance()->GetType(varname) == CType_int) {
			CInt tmp(varname);
			sprintf(vardata, "%i", *tmp); 
		} else if(CMap::instance()->GetType(varname) == CType_float) {
			CFloat tmp(varname);
			sprintf(vardata, "%i", *tmp); 
		} if(CMap::instance()->GetType(varname) == CType_vec2) {
			CVec2 tmp(varname);
	...
	bool vcount = CMap::instance()->DoesExist(strippedLH);
	if(!vcount) {
		char buffer[128];
		sprintf(buffer, "Identifier not found \"%s\"\n", strip(LH));
		std::string out(buffer);
		...
		CType type = CMap::instance()->GetType(strippedLH);
		if(type == CType_int) {
			CInt tmp(strippedLH);
			*tmp = atoi(strippedRH);
		} else if(type == CType_float) {
